...
export const handlerQuery = (GQL, query, options, auth) => {
  return () => {
    ...
    let reqQuery = useQuery(
      GQL.select[query],
      Object.assign(
        {},
        MergeRecursive(
          GQL.options && GQL.options[query] ? GQL.options[query] : {},
          options
        )
      )
    );
    useEffect(() => {
      if (reqQuery.error) {
        notification["error"]({
          message: reqQuery.error.message,
          duration: 0,
  ...
  const UserObject = {
    ...
    documentControl: {
      ...
      orders: {
        select: admin
          ? true
          : accesses.includes("/document-control-p/orders-p/select"),
      },
      revisedUser: {
        select: admin
          ? true
          : accesses.includes("/document-control-p/reviseduser-p/select"),
      },
      approvedUser: {
        select: admin
          ? true
      ...
      rejectedUser: {
        select: admin
          ? true
          : accesses.includes("/document-control-p/rejecteduser-p/select"),
      },
      ...
      onApproval: {
        select: admin
          ? true
          : accesses.includes("/document-control-p/on-approval-p/select"),
      },
      onApprovalList: {
        select: admin
          ? true
          : accesses.includes("/document-control-p/on-approval-list-p/select"),
      },
      forExecutionInbox: {
        select: admin
          ? true
          : accesses.includes(
              "/document-control-p/for-execution-inbox-p/select"
            ),
      },
      approved: {
        select: admin
      ...
      isItemStatusChangeAllowed: admin
        ? true
        : accesses.includes("/document-control-p/item-status-change"),
    },
    documentReport: {
      ...
      isDocumentStatusChangeAllowed: admin
        ? true
        : accesses.includes("/document-report-p/document-status-change"),
      isItemStatusChangeAllowed: admin
        ? true
        : accesses.includes("/document-report-p/item-status-change"),
    },
    documentHistory: {
      ...
      isDocumentStatusChangeAllowed: admin
        ? true
        : accesses.includes("/document-history-p/document-status-change"),
      isItemStatusChangeAllowed: admin
        ? true
        : accesses.includes("/document-history-p/item-status-change"),
    },
    documentSearch: {
      ...
      search: {
        select: admin
          ? true
          : accesses.includes("/document-search-p/search-p/select"),
      },
      isDocumentStatusChangeAllowed: admin
        ? true
        : accesses.includes("/document-search-p/document-status-change"),
      isItemStatusChangeAllowed: admin
        ? true
        : accesses.includes("/document-search-p/item-status-change"),
    },
    adminDepartment: {
      ...
      isDocumentStatusChangeAllowed: admin
        ? true
        : accesses.includes("/admin-p/document-status-change"),
      isItemStatusChangeAllowed: admin
        ? true
  ...
  let accessRedirect = (props) => {
    ...
    switch (pathname) {
      ...
      case "/admin/positions-page/":
        ...
        break;
      case "/admin/document-statuses-page":
      case "/admin/document-statuses-page/":
        if (!user.admin) {
          return <Redirect />;
        }
        break;
      case "/admin/document-routes-page":
      case "/admin/document-routes-page/":
        if (!user.admin) {
          return <Redirect />;
        }
        break;
      case "/admin/forms-settings-page":
      case "/admin/forms-settings-page/":
        if (!user.admin) {
          return <Redirect />;
        }
        break;
      case "/admin/department-page":
      case "/admin/department-page/":
        if (!user.admin) {
          return <Redirect />;
        }
      ...
      case "/document-control/":
        if (!user.documentControl.select) {
          return <Redirect />;
        }
        break;
      ...
      case "/document-control/reviseduser/":
        if (!user.documentControl.approvedUser.select) {
          return <Redirect />;
        }
        break;
      ...
      case "/document-control/on-approval/":
        if (!user.documentControl.onApproval.select) {
          return <Redirect />;
        }
        break;
      case "/document-control/on-approval-list":
      case "/document-control/on-approval-list/":
        if (!user.documentControl.onApprovalList.select) {
          return <Redirect />;
        }
        break;
      ...
      case "/document-control/for-execution-inbox/":
        if (!user.documentControl.forExecutionInbox.select) {
          return <Redirect />;
        }
        break;
      ...
      case "/document-report/":
        if (!user.documentReport.select) {
          return <Redirect />;
        }
        break;
      case "/document-history":
      case "/document-history/":
        if (!user.documentHistory.select) {
          return <Redirect />;
        }
        break;
      ...
      case "/document-search/":
        if (!user.documentSearch.select) {
          return <Redirect />;
        }
        break;
      case "/document-search/search":
      case "/document-search/search/":
...
export const getFirstMonthDate = (anydate = new Date()) => {
  ...
  let today = new Date(
    today_tmp.getTime() - today_tmp.getTimezoneOffset() * 60 * 1000
  );
  let myDate = today;
...
/**
 * @description Создает всплывающее в браузере окно с сообщением
 * @param {*} text
 */
