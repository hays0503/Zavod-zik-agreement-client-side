...
const FormSettingsPage = React.memo((props) => {
  const user = useUser()
  ...
  const titleMenu = (tableProps) => {
    return (<TitleMenu
            ...
            buttons={[
				...
				<Popconfirm
                    title="Вы уверены?"
                    onConfirm={() => { const variables = {}; variables[forms.exemplar] = { id: Number(tableProps.selectedRowKeys[0]), log_username: user.username }; remove({ variables }) }}
                    okText="Да"
                    cancelText="Нет"
...
const FormSettingsForm = React.memo((props) => {
  const user = useUser()
