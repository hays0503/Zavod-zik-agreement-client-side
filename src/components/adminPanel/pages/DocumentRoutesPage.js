...
import {
	...
	FormOutlined,
	CloseSquareOutlined,
} from "@ant-design/icons"; //quick actions panel icon
...
let DocumentRoutesPage = React.memo((props) => {
	...
	const [remove, { loading: loadingRemove }] = handlerMutation(
		useMutation(document_routes.delete)
	)();
			...
			<TitleMenu
				...
				buttons={[
					...
					<Popconfirm
						...
						onConfirm={() => {
							let variables = {};
							variables[document_routes.exemplar] = {
	...
	useEffect(() => {
		if (props.initialValues) {
			setState({
				...
				status_in_process:
					props.initialValues.document_routes[0].status_in_process,
				status_cancelled:
					props.initialValues.document_routes[0].status_cancelled,
			...
			>
				...
				>
					<Input
						...
						placeholder="Название маршрута документа"
					/>
				</Form.Item>
				<Form.Item
				...
				>
					<IndependentSelect
						disabled={props.disabled}
						placeholder="Статус в процессе"
						title="Статус в процессе"
						query={document_statuses}
					/>
				</Form.Item>
				<Form.Item
				...
				>
					<IndependentSelect
						disabled={props.disabled}
						placeholder="Статус - отклонён"
						title="Статус - отклонён"
						query={document_statuses}
					/>
				</Form.Item>
				<Form.Item
				...
				>
					<IndependentSelect
						disabled={props.disabled}
						placeholder="Статус - завершён"
						title="Статус - завершён"
						query={document_statuses}
					/>
				</Form.Item>
					...
					<span style={{ marginRight: "5px" }}>
						{props.setViewMode ? (
							...
							<Popconfirm
								...
								icon={<QuestionCircleOutlined style={{ color: "red" }} />}
							>
								<Button title="Сохранить" icon={<DeleteOutlined />} />
							</Popconfirm>
						)}
					</span>
					<span style={{ marginRight: "5px" }}>
