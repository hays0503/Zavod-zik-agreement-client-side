...
const ModalInsert = React.memo(({ GQL, GQL2, GQL3, GQL4, GQL5, Form1, Form2, Form3, Form4, Form5, ...props }) => {
  ...
  useEffect(() => {
    if (dataRoutes && dataRoutes[Object.keys(dataRoutes)[0]] != null && state.route_id > 0) {
      ...
      setRoutesList(routesMap = (dataRoutes.document_routes[0].routes !== undefined)
        ? dataRoutes.document_routes[0].routes.map((item) => {
          return {
