   ...
   ({
      ...
      useEffect(() => {
         if (
            ...
            let boss = dataRoutes.document_routes[0].routes.find(
               (route, index) => {
                  //Оповещение о ошибке у данного пользователя нет начальника
                  if (dataBoss?.get_boss_depart[0]?.positions[0] == undefined) {
                     throw new UserException(
                        "У данного пользователя нет начальника"
                     );
                  }
                  return (
                     route.positionId ==
                     dataBoss?.get_boss_depart[0]?.positions[0]
                  );
               }
            );
               ...
               >
                  ...
                  <div className={cRef.current}>
                     <span className="marginBottom displayBlock">
                        Для создания нового маршрута, укажите все необходимые
                        должности в нужном порядке
                     </span>
