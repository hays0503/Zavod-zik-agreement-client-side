...
const PrintForm3 = React.forwardRef((props, ref) => {
	...
	let generateQRCode = async () => {
        ...
        Валюта платежа: ${props?.printData?.documentData?.documents[0]?.data_agreement_list_production[0]?.currency},
        Наименование подразделения, фамилия ответственного исполнителя: ${props?.printData?.documentData?.documents[0]?.data_agreement_list_production[0]?.executor_name_division},
        Телефон исполнителя: ${props?.printData?.documentData?.documents[0]?.data_agreement_list_production[0]?.executor_phone_number}
