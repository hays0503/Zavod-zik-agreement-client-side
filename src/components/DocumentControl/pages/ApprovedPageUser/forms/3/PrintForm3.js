...
const PrintForm3 = React.forwardRef((props, ref) => {
	...
	return (
		<div ref={ref}>
			...
			<div className="page">
				<div style={{ paddingLeft: "25px", paddingRight: "30px" }}>
					...
					<div>
						...
						<div style={{ marginBottom: "10px" }}>
							<b>Наименование контрагента:</b> {props?.printData?.documentData?.title}
						</div>
						<div style={{ marginBottom: "10px" }}>
							<b>Предмет договора:</b> {props?.printData?.documentData?.data_agreement_list_production[0]?.subject}
						</div>
						<div style={{ marginBottom: "10px" }}>
							<b>Общая сумма договора:</b> {props?.printData?.documentData?.data_agreement_list_production[0]?.price}
						</div>
						<div style={{ marginBottom: "10px" }}>
							<b>Валюта платежа:</b> {props?.printData?.documentData?.data_agreement_list_production[0]?.currency}
						</div>
						<div style={{ marginBottom: "10px" }}>
							<b>
								Наименование подразделения, фамилия ответственного исполнителя:
							</b>{" "}
							{props?.printData?.documentData?.data_agreement_list_production[0]?.executor_name_division}
						</div>
						<div style={{ marginBottom: "10px" }}>
							<b>Телефон исполнителя:</b>{" "}
							{props?.printData?.documentData?.data_agreement_list_production[0]?.executor_phone_number}
						</div>
						<div style={{ marginBottom: "10px" }}>
							<b>Контакты контрагента:</b>{" "}
							{props?.printData?.documentData?.data_agreement_list_production[0]?.counteragent_contacts}
						</div>
					</div>
					<div className="subpagepage">
						...
						<div style={{ paddingTop: "20px" }}>
							<b>Исполнитель:</b> {props?.printData?.documentData?.position}, {props?.printData?.documentData?.fio} <br />
			...
			{printReasons(props?.printData?.documentData)}
