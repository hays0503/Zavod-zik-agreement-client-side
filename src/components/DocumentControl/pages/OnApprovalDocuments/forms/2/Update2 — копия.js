...
let Update2 = React.memo((props) => {
    ...
    function confirm() {
        if (reasonText != null || reasonText?.length > 0) {
            setState(prevState => {
                ...
                old.reason = null;
                old.reason = props.initialValues2.documents[0].reason ? props.initialValues2.documents[0].reason : [];
    ...
    let HandleReasonChange = (all, change) => {
    ...
    return (
        ...
        >
            ...
            <Row>
                <Col span={24}>
                    <Divider type={'horizontal'} />
                    <Popconfirm disabled={props.disabled} placement="topLeft" title={confirmText} onConfirm={confirm} okText="Да" cancelText="Нет">
                        <Button disabled={props.disabled} type='primary' onClick={Approve}>
                    ...
                    <Space>
                        {/* <Divider type={'vertical'} />
                        <Button type="primary" htmlType="submit" disabled={props.disabled} onClick={props.handleRouteBackward2}>Вернуть на доработку</Button>
                        <Divider type={'vertical'} /> */}
                        <Popconfirm disabled={props.disabled} placement="topLeft" title={confirmText} visible={rejectVisible} onCancel={()=>{setRejectVisible(false)}} onConfirm={rejectConfirm} okText="Да" cancelText="Нет">
                            <Button type="primary" disabled={props.disabled} onClick={Reject}>
                                Отклонить
                            </Button>
                        </Popconfirm>
                    </Space>
            ...
            <div>
                <Input disabled={props.disabled} onChange={HandleReasonChange} placeholder="Замечание" />
