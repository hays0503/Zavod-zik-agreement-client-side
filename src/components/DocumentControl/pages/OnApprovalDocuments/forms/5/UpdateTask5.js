    ...
    return (
        ...
        >
            ...
            {(state?.document_options?.subject === true) ?
                <FormWrap>{FormItem("Основание: ",props?.document?.data_agreement_list[0]?.subject)}</FormWrap>: ''
            }
            {/* /////////////////////////////////// */}
            {(state?.document_options?.remark === true) ?
                <FormWrap>{FormItem("Примечание: ",props?.document?.data_custom[0]?.remark)}</FormWrap>: ''                
            }
