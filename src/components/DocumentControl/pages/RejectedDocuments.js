			...
			<TitleMenu
				buttons={[
					<ModalUpdate
						...
						GQL={documents}
						GQL2={documents}
						UpdateForm={DocumentsFormUpdate}
						UpdateForm2={DocumentsFormUpdate2}
						update={true}
						width={750}
					/>,
					<Popconfirm
	...
	useEffect(() => {
		if (props.initialValues2) {
			setState({
				...
				description:
					props.initialValues2.documents[0].data_agreement_list[0].description,
		...
		>
			...
			>
				<Input
					disabled={props.disabled}
					className="form-input"
					placeholder="В валюте цены договора"
				/>
			</Form.Item>
			<Form.Item
				name="currency_price"
			...
			>
				<Input
					disabled={props.disabled}
					className="form-input"
					placeholder="В тенге, по курсу НБ РК"
				/>
			</Form.Item>
			<Form.Item
