...
import { FragmentSwitchArea, FragmentSwitchAreaArray } from "./../pages/fragments/FragmentSwitchArea";
import {
			...
			variables: {
				document_tasks: {
					...
					document_options: {
						...
						custom_area:
							values.custom_area == undefined ? false : values.custom_area,
					},
		...
		<>
			...
			>
				...
				>
					...
					{console.log(
						"props?.document?.documents[0]?.data_custom[0]?.custom_area",
						props?.document?.documents[0]?.data_custom[0]?.custom_area
					)}
					...
					фрагмент antd дающий возможность выбрать через switch
					какое поле будет в документе */}
					<FragmentSwitchAreaArray
						data={props?.document?.documents[0]?.data_custom[0]?.custom_area}
						name={"custom_area"}
						label={"Все дополнительные поля: "}
					/>
